...
function Ovainum:Helpful()
	--Destroy Old
	if CoreGui:FindFirstChild("Ovainum Utility Hub") then
		CoreGui:FindFirstChild("Ovainum Utility Hub"):Destroy()
	end
	...
	local MainFrameGrid = Instance.new("UIGridLayout")
	local PlayerTrack = Instance.new("TextLabel")
	local GravityTrack = Instance.new("TextLabel")
	local TimeTrack = Instance.new("TextLabel")
	local FPSTrack = Instance.new("TextLabel")
	local HealthTrack = Instance.new("TextLabel")
	local PositionTrack = Instance.new("TextLabel")
	local TeamTrack = Instance.new("TextLabel")
	...
	MainFrameGrid.Parent = MainFrame
	PlayerTrack.Parent = MainFrame
	GravityTrack.Parent = MainFrame
	TimeTrack.Parent = MainFrame
	FPSTrack.Parent = MainFrame
	HealthTrack.Parent = MainFrame
	PositionTrack.Parent = MainFrame
	TeamTrack.Parent = MainFrame

	--Properties
	Main.Name = "Ovainum Utility Hub"
	Main.IgnoreGuiInset = true
	Main.DisplayOrder = 1999999999
	...
	MainFrameGrid.CellPadding = UDim2.new(0, 0, 0, 0)
	MainFrameGrid.CellSize = UDim2.new(1, 0, 0.1, 0)

	PlayerTrack.Name = "Player Tracker"
	PlayerTrack.BackgroundTransparency = 1
	PlayerTrack.Text = "Players: nil"
	PlayerTrack.Font = Enum.Font.Arial
	PlayerTrack.TextScaled = true
	PlayerTrack.TextColor3 = Color3.fromRGB(255, 255, 255)
	for _, plr in players:GetPlayers() do
		Player_Amount += 1
	end
	PlayerTrack.Text = "Players: "..Player_Amount

	GravityTrack.Name = "Gravity Tracker"
	GravityTrack.BackgroundTransparency = 1
	GravityTrack.Text = "Gravity: "..game:GetService("Workspace").Gravity
	GravityTrack.Font = Enum.Font.Arial
	GravityTrack.TextScaled = true
	GravityTrack.TextColor3 = Color3.fromRGB(255, 255, 255)

	TimeTrack.Name = "Time Tracker"
	TimeTrack.BackgroundTransparency = 1
	TimeTrack.Text = "Time: "..lighting.TimeOfDay
	TimeTrack.Font = Enum.Font.Arial
	TimeTrack.TextScaled = true
	TimeTrack.TextColor3 = Color3.fromRGB(255, 255, 255)

	FPSTrack.Name = "FPS Tracker"
	FPSTrack.BackgroundTransparency = 1
	FPSTrack.Text = "FPS: nil"
	FPSTrack.Font = Enum.Font.Arial
	FPSTrack.TextScaled = true
	FPSTrack.TextColor3 = Color3.fromRGB(255, 255, 255)

	HealthTrack.Name = "Health Tracker"
	HealthTrack.BackgroundTransparency = 1
	HealthTrack.Text = "Health: "..hum.Health
	HealthTrack.Font = Enum.Font.Arial
	HealthTrack.TextScaled = true
	HealthTrack.TextColor3 = Color3.fromRGB(255, 255, 255)

	PositionTrack.Name = "Position Tracker"
	PositionTrack.BackgroundTransparency = 1
	PositionTrack.Text = "Position x/y/z: nil"
	PositionTrack.Font = Enum.Font.Arial
	PositionTrack.TextScaled = true
	PositionTrack.TextColor3 = Color3.fromRGB(255, 255, 255)

	TeamTrack.Name = "Team Tracker"
	TeamTrack.BackgroundTransparency = 1
	TeamTrack.Text = "Team: nil"
	TeamTrack.Font = Enum.Font.Arial
	TeamTrack.TextScaled = true
	TeamTrack.TextColor3 = Color3.fromRGB(255, 255, 255)
	if Ovainum:TeamCheck(LP) == "No Team" then
		TeamTrack.Text = "Team: "..Ovainum:TeamCheck(LP)
	else
		TeamTrack.Text = "Team: "..LP.Team
	end
	...
	game:GetService("Workspace"):GetPropertyChangedSignal("Gravity"):Connect(function()
		GravityTrack.Text = "Gravity: "..game:GetService("Workspace").Gravity
	end)
	...
	spawn(function()
		while wait() do
			local hum = LP.Character:WaitForChild("Humanoid")
			if hum.Health > 0 and LP.Character:FindFirstChild("HumanoidRootPart") then
				PositionTrack.Text = "Position x/y/z: "..math.round(LP.Character.HumanoidRootPart.Position.X).."/"..math.round(LP.Character.HumanoidRootPart.Position.Y).."/"..math.round(LP.Character.HumanoidRootPart.Position.Z)
			end
		end
	end)
